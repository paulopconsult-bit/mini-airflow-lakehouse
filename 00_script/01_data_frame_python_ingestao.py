...
def criar_primeiro():
    ...
    # 4. Quantidade aleatória de registros (maioria entre 6k e 10k)
    import numpy as np
    total_registros = int(abs(np.random.normal(8000, 1500)))

    # Garante mínimo de 3000 e máximo de 15000
    total_registros = max(3000, min(total_registros, 15000))

    registros = []

    # Viés proporcional para id_pessoa (natural e suave)
    id_pessoas = list(range(1, 101))
    pesos_pessoas = [1/(p**0.7) for p in id_pessoas]
    soma_pesos = sum(pesos_pessoas)
    pesos_pessoas = [w / soma_pesos for w in pesos_pessoas]
    ...
    for i in range(1, total_registros + 1):
        ...
        registros.append({
            ...
            "id_pessoa": random.choices(id_pessoas, pesos_pessoas, k=1)[0],
            "id_produto": random.randint(1, 4),
            "classe": random.choices([1, 2, 3], weights=[0.6, 0.3, 0.1], k=1)[0],
    ...
    print(f"Primeiro arquivo criado com {total_registros} registros:", caminho)
...
def incrementar(caminho):
    ...
    # Viés proporcional para id_pessoa (mesmo da seção 5)
    id_pessoas = list(range(1, 101))
    pesos_pessoas = [1/(p**0.7) for p in id_pessoas]
    soma_pesos = sum(pesos_pessoas)
    pesos_pessoas = [w / soma_pesos for w in pesos_pessoas]
